...
const copyManifest = () => gulp.src('source/manifest.webmanifest').pipe(gulp.dest('build'));
...
const syncServer = () => {
  ...
  gulp.watch('source/img/favicons/**', gulp.series(copy, copySvg, refresh));
  gulp.watch('source/**.ico', gulp.series(copy, refresh));
  gulp.watch('source/video/**', gulp.series(copy, refresh));
  gulp.watch('source/downloads/**', gulp.series(copy, refresh));
  gulp.watch('source/*.php', gulp.series(copy, refresh));
  gulp.watch('source/manifest.webmanifest', gulp.series(copyManifest, refresh));
};

const build = gulp.series(clean, copy, sprite, gulp.parallel(styles, js, optimizePng, optimizeJpg, optimizeSvg, copyManifest));
